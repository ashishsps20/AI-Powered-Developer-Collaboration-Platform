...
const ReassignManagerModal = ({ organizationId, projectId, isOpen, onClose }) => {
  const [userId, setUserId] = useState('');
  const [members, setMembers] = useState([]);
  const [loadingMembers, setLoadingMembers] = useState(false);
  const [localError, setLocalError] = useState('');

  const { reassignProjectManager, isLoading, error, clearError } = useProjectStore();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!userId) {
      setLocalError('Please select a new Project Manager.');
      return;
    }

    try {
      await reassignProjectManager(organizationId, projectId, userId);
      onClose();
    } catch (err) {
      // Handled by store
    }
        ...
        <div className="relative z-10 inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <form onSubmit={handleSubmit}>
            <div className="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
              <h3 className="text-lg font-medium leading-6 text-gray-900 mb-4">Change Project Manager</h3>
              
              <div className="mb-4 bg-yellow-50 border-l-4 border-yellow-400 p-4">
                <p className="text-sm text-yellow-700">
                  Assigning a new Project Manager will demote the current manager(s) to DEVELOPER.
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="manager" className="block text-sm font-medium text-gray-700">Select New Project Manager</label>
                  {loadingMembers ? (
                    <p className="text-sm text-gray-500 mt-1">Loading organization members...</p>
                  ) : (
                    <select
                      id="manager"
                      value={userId}
            ...
            <div className="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              >
                {isLoading ? 'Reassigning...' : 'Assign New Manager'}
              </button>
              <button
                type="button"
                onClick={onClose}
